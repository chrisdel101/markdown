...
export const onDeleteSpaceAndLineFormatter = (strSplitOnNewLines: string[], currentLineNumber: number) => {
  // console.log('current');
  return strSplitOnNewLines.map((line, index) => {
    ...
    if (strSplitOnNewLines[index + 1] !== undefined) {
      ...
      } else if (regex.isOrderedListIndexAtLineStart.test(line) || regex.isListSymbolAtLineStart.test(line)) {
        return `${line.trim()}\n`;
        // if not list items
      } else {
        // if not whitespace
...
export const onAddSpaceLineFormatter = (strSplitOnNewLines: string[], listType?: ListTypes) => {
  ...
  return strSplitOnNewLines.map((line, index) => {
    ...
    if (aloneRegex(line)) {
      ...
      if (strSplitOnNewLines[index - 1] !== 'undefined' && aloneRegex(strSplitOnNewLines[index - 1])) {
...
export const removePreviousSymbols = (
  ...
  for (let counter = ind; inputArr[counter] === '#' || inputArr[counter] === ` `; counter++) {
...
export const indexInsideInputString = (indexesArr: number[], cursorIndexes: CursorState) => {
  // default to zero
  let clickedIndex = 0;
...
export const getCurrentLine = (indexesArr: number[], cursorIndexes: CursorState) => {
...
// used to update state of currnetLineNumber
export const getCurrentLineInsideList = (list: List, cursorIndexes: CursorState) => {
...
export const getStartIndexesOfEachLineArr = (splitInputOnNewlines: string[], spacesToAdd: number) => {
...
export const getCurrentListIndex = (listsArr: List[], cursorIndexes: CursorState) => {
  ...
  for (let index = 0; index < listRanges.length; index++) {
    ...
    if (cursorIndexes.startIndex >= startIndex && cursorIndexes.startIndex <= (endIndex || 0)) {
...
export const getInputValueLineNumber = (splitInputOneNewLines: string[], cursorIndexes: CursorState) => {};
export const isCursorInsideList = (listsArr: List[], cursorIndexes: CursorState) => {
  return getCurrentListIndex(listsArr, cursorIndexes) !== undefined ? true : false;
};
...
export const checkListContentIndex = (contentToCheck?: string, inputStr?: string) => {
...
export const getFirstLineIndex = (splitInputOnNewlines: string[], list: List) => {
...
export const calculateNewList = (list: List, newStartIndex: number, splitInputOneNewLines: string[]) => {
...
export const adjustListIndexes = (listsArr: List[], inputValue: string, activeListIndex?: number): List[] | [] => {
  ...
  const newStartIndexes = listsArr.map((list, i) => {
    // console.log('i', i);
    ...
    if (i !== activeListIndex) {
      if (list?.content) {
        ...
        if (findListStartIndex !== list.startIndex) {
          ...
          const newList = calculateNewList(listsArr[i], findListStartIndex, splitInputOnNewlines);
          // console.log('new list', newList);
          return newList;
        } else {
          return list;
