...
import { getStartIndexesOfEachLineArr, onAddSpaceLineFormatter, isNumber, regex } from '../markDownInputUtils';
...
export class List {
  ...
  setEndIndex = (index: number, type: ListIndexSetter) => {
    ...
    if (type === ListIndexSetter.Set) {
      const itemIndexLen = ListTypes.list.valueOf() ? `* `.length : `${this.itemIndexes.length}. `.length;
  ...
  writeLineIndex = () => {
    return this.listType === ListTypes.list.valueOf() ? `* ` : `${this.itemIndexes.length}. `;
  };
}
...
export const createList = ({
  ...
  const contenWhenNextLineLength =
    ...
    newListLineIndexes?.[startOfNextLineIndexInArr] - newListLineIndexes?.[currentLineNumber];
  ...
  let contentWhenNoNextLineLength = splicedInputArrToStr.length - newListLineIndexes[currentLineNumber];
  ...
  let newContentStr = splicedInputArrToStr
    .slice(
      ...
      newListLineIndexes?.[currentLineNumber] + (contenWhenNextLineLength ?? contentWhenNoNextLineLength)
    )
    ?.trim();
  // 4.log('CONTENT', newContentStr);
  // 4.log('split CONTENT', newContentStr.split('\n'));
  const startOfLineIndexInArrValue = newListLineIndexes?.[currentLineNumber] ?? 0;
  // 4.log('startOfLineIndexInArrValue', startOfLineIndexInArrValue);
  let endIndex =
    startOfLineIndexInArrValue +
    (contenWhenNextLineLength ? contenWhenNextLineLength - 1 : contentWhenNoNextLineLength);
  // 4.log('END', endIndex);
  ...
  const returnValues: CreateListOutput = {
    ...
    _activeListIndexState: {
      ...activeListIndexState,
      currentListIndex: isNumber(activeListIndexState?.prevListIndex) ? activeListIndexState?.prevListIndex! + 1 : 0,
    },
  };
...
export const updateList = ({
  ...
  const sliceInputValueStart = splitInputOnNewlines.slice(currentList.lineNumberStart);
  // console.log('sliceInputValueStart input', sliceInputValueStart);
  // trim empty arr elements off end
  const sliceInputValueEnd = sliceInputValueStart.slice(0, currentList.itemIndexes?.slice(-1)[0] + 1).filter(Boolean);
  // console.log('sliceInputValueStart input', sliceInputValueStart);
  ...
  const currentLineListContentStr = currentList?.content?.[lineNumInCurrentList] || '';
  ...
  if (currentLineInputValue !== currentLineListContentStr) {
    ...
    const sliceLineIndexesStart = inputLineIndexes?.slice(currentList.lineNumberStart);
    const sliceLineIndexesEnd = sliceLineIndexesStart?.slice(0, currentList.itemIndexes?.length);
    ...
    const currentInputLineLen = currentLineInputValue?.length > 0 ? currentLineInputValue?.length : 0;
    const currentContentLineLen = currentLineListContentStr?.length > 0 ? currentLineListContentStr?.length : 0;
...
export const breakOutList = ({
  ...
  let newInputValue = `${onAddSpaceLineFormatter(splitInputOnNewlines, listType).join('')}`;
  ...
  let obj = {
    ...
    lineIndexes: currentList.lineIndexes?.slice(0, currentList.itemIndexes.length - 1),
...
export const continueList = ({
  ...
  console.log('------MIDDLE--------');
  console.log('splitInputOnNewlines', splitInputOnNewlines);
  const currentListCopy = { ...listsArr[activeListIndexState.currentListIndex || 0] };
  ...
  if (listType === ListTypes.listOl.valueOf()) {
    splitInputOnNewlinesCopy.splice(currentLineNumber + 1, 0, currentListCopy.writeLineIndex());
  } else if (listType === ListTypes.list.valueOf()) {
    splitInputOnNewlinesCopy.splice(currentLineNumber + 1, 0, currentListCopy.writeLineIndex());
  }
  const sliceInputValueStart = splitInputOnNewlinesCopy.slice(currentListCopy.lineNumberStart);
  // console.log('sliceInputValueStart input', sliceInputValueStart);
  let sliceInputValueEnd = sliceInputValueStart.includes('')
    ? // white space is the cut off point
      sliceInputValueStart.slice(0, sliceInputValueStart.indexOf('')).filter(Boolean)
  ...
  currentListCopy.lineIndexes = [...(currentListCopy?.lineIndexes || []), indexesArr[currentLineNumber + 1]];
  ...
  if (
    regex.isOrderedListItemAloneOnLine.test(newContentArr[currentListCopy.itemIndexes.length - 1]) ||
    regex.isListItemAloneOnLine.test(newContentArr[currentListCopy.itemIndexes.length - 1])
  ) {
    ...
    if (newContentArr[currentListCopy.itemIndexes.length] !== undefined) {
      ...
      if (newContentArr[currentListCopy.itemIndexes.length - 2] !== undefined) {
        if (
          regex.isOrderedListItemAloneOnLine.test(newContentArr[currentListCopy.itemIndexes.length - 2]) ||
          regex.isListItemAloneOnLine.test(newContentArr[currentListCopy.itemIndexes.length - 2])
        ) {
          // console.log('currentList endIndex', currentListCopy.endIndex);

          // trim any list indexes alone on line - trim the 1 - ['1.', '2. hello']
          const itemIndexAlone = newContentArr[currentListCopy.itemIndexes.length - 1];
      ...
      const combineTwoItems = `${newContentArr[currentListCopy.itemIndexes.length - 1]}${
        newContentArr[currentListCopy.itemIndexes.length]
      }`;
      // console.log('combine', combineTwoItems);
      const addToEndIndex = newContentArr[currentListCopy.itemIndexes.length].length;
      // console.log('combine', combineTwoItems);
  ...
  const newList = {
    ...
    endIndex: listType === ListTypes.listOl.valueOf() ? currentListCopy.endIndex + 3 : currentListCopy.endIndex + 2,
  };
  // console.log('newList', newList);

  const addNewLineCharsArr = onAddSpaceLineFormatter(splitInputOnNewlinesCopy, listType);
